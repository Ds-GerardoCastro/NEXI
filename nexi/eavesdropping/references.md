...
- **Veličković, P., Cucurull, G., Casanova, A., Romero, A., Liò, P., & Bengio, Y. (2018).** _Graph Attention Networks._ International Conference on Learning Representations.
  _Architectural analog: agents (nodes) attend over edges in which they don't directly participate. The attention mechanism over a node's neighbourhood is structurally homologous to eavesdropping in a multi-agent setting._
...
- **Foerster, J. N., Song, F., Hughes, E., Burch, N., Dunning, I., Whiteson, S., Botvinick, M., & Bowling, M. (2019).** _Bayesian Action Decoder for Deep Multi-Agent Reinforcement Learning._ International Conference on Machine Learning.
  _Belief modelling in multi-agent RL — agents reason about others' private information through observed actions. Closely related to the eavesdropping use-case in cooperative tasks._

- **Oliehoek, F. A., & Amato, C. (2016).** _A Concise Introduction to Decentralized POMDPs._ Springer Briefs in Intelligent Systems.
  _Theoretical framing for partially observable multi-agent decision making. Provides the formal substrate for analyzing what eavesdropping changes about the agent's information state._
