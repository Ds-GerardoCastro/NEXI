...
- **Covered** — the corpus already documents this function-in-this-substrate.
- **Field-known** — real research exists but has not been pulled in yet → **the search targets**.
- **Frontier** — enumerated by the taxonomy with no known research → genuine white space.

A third **bridge** view aligns the two grids on the shared function axis: functions that are **hot in nature but cold in AI** are the highest-value places to mine for new patterns (a visual, quantified companion to the same nature→AI bridge the catalog is built on).

The map is a single self-contained HTML heat map, viewable as a live page: **[ds-gerardocastro.github.io/NEXI/coverage-grid.html](https://ds-gerardocastro.github.io/NEXI/coverage-grid.html)** (source: [`docs/coverage-grid.html`](docs/coverage-grid.html)). It is regenerated by `python tools/coverage-grid/build.py`, which reads the maintainer's upstream research vaults and writes both the tool copy and the published `docs/` page. The reference axes live in [`taxonomy.yaml`](tools/coverage-grid/taxonomy.yaml); the committed matrix + HTML are the published snapshot.
...
- The [**design-space coverage map**](https://ds-gerardocastro.github.io/NEXI/coverage-grid.html) (above) — a coverage-driven, non-momentum way to choose what to research next.
