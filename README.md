...
A third **bridge** view aligns the two grids on the shared function axis: functions that are **hot in nature but cold in AI** are the highest-value places to mine for new patterns (a visual, quantified companion to the same nature→AI bridge the catalog is built on). Output is a single self-contained HTML heat map — [`tools/coverage-grid/coverage-heatmap.html`](tools/coverage-grid/coverage-heatmap.html) — regenerated by `python tools/coverage-grid/build.py`. The reference axes live in [`taxonomy.yaml`](tools/coverage-grid/taxonomy.yaml); regeneration reads the maintainer's upstream research vaults, and the committed matrix + HTML are the published snapshot.
...
- The [**design-space coverage map**](tools/coverage-grid/) (above) — a coverage-driven, non-momentum way to choose what to research next.
