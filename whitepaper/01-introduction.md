...
Section 2 surveys related work in world models, scaling debates, comparative cognition, and pattern-catalog approaches in AI. Section 3 develops the two-pillar critique in detail and establishes the falsifiability of the program. Section 4 describes the methodology — how peer-reviewed sources are processed into design primitives, with curation discipline that preserves provenance and enforces falsifiability. Section 5 introduces the NEXI catalog framework: schema, layered cards, the cluster concept, status fields, and the three structural cluster shapes (constellation, pipeline, composition) that emerged from catalog construction. Section 6 presents three demonstration case studies — one per cluster shape: the **Distributed Social Cognition** cluster (constellation) derived from research on wild zebra finch communication networks; the **Bounded Cognitive Architecture** cluster (pipeline) derived from work on cognitive-capacity limits in non-human animals; and the **Acerebrate Decision-Making** cluster (composition) derived from research on bacterial decision-making without nervous systems. Each case study reports its member patterns, its system-level falsifiable hypothesis, and the limitations honestly attached to single-source or multi-source evidence. Section 7 discusses limitations, falsifiability of the overall program, relationship to mainstream AI research, and engagement with counter-arguments. Section 8 outlines future work. Section 9 concludes.
