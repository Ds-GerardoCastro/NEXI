...
The three clusters together exercise the framework against three distinct shapes, three sources, and three problem classes. The discussion in Section 7 returns to what they collectively support and what they leave open. Section 6.6 below summarises the fourth cluster (Embodied Action Selection, isomorphism shape), ingested after the three case studies were structured, which extends the framework's cluster-shape catalogue by one and provides the catalog's second canonical NEXI on cross-substrate evidence. Section 6.7 summarises the fifth cluster (Convergent Intelligence Attractor, convergence-fan shape), ingested later still, which extends the catalogue by a further shape.
...
## 6.7 Addendum: Convergent Intelligence Attractor (convergence-fan cluster)

A fifth cluster, **Convergent Intelligence Attractor**, was ingested after the fourth. Like §6.6 it is presented as an addendum rather than a co-equal case study, because it postdates the case-study selection that produced the §5.6 / §6.4 shape findings. It matters to the framework for a specific reason: it is the first cluster whose shape is the deliberate inverse of an existing one, which tests whether the shape vocabulary can represent opposition and not only variety.

**Source and provenance.** The cluster is anchored to [Roth 2015], a peer-reviewed comparative synthesis in _Philosophical Transactions of the Royal Society B_ (vol. 370, art. 20150049) documenting that complex brains and high intelligence evolved independently across taxa whose last common ancestor had neither — insect mushroom bodies, cephalopod vertical lobe, teleost everted pallium, avian nidopallium, primate isocortex. Two comparative regularities do the load-bearing work: intelligence is invariantly bound to a discrete multimodal associative centre regardless of the underlying anatomy, and it tracks network-quality metrics (neuron number, packing density, connectivity) rather than absolute brain size — large-brained elephants and cetaceans rank below denser-networked corvids and primates.

**Shape: convergence-fan.** The cluster's three members are organised as a fan of independent routes converging on one functional attractor: `convergence-fan-routes` (the topology — the same function reached by non-homologous machinery across phyla), `convergent-integration-hub` (the invariant the routes converge on — a dense multimodal associative locus), and `network-density-over-size` (the metric by which the invariant is measured and falsified). This is categorically distinct from the four prior shapes, and specifically the inverse of isomorphism: isomorphism is one conserved mechanism redeployed under domain shift; convergence-fan is many non-homologous mechanisms independently reaching one functional target. The catalog's framework absorbs it as the fifth cluster shape (§5.6).

**System-level falsifiable hypothesis.** If high-level cognitive capability is a convergent functional attractor, AI systems that target the substrate-invariant signature the convergence reveals — a dedicated dense associative integration hub, sized by connectivity and packing rather than raw parameter count — should reach comparable capability from architecturally diverse starting points at matched connectivity budget, while scaling absolute size without a convergent integration hub should underperform. Refutation: if capability tracks absolute scale rather than the presence and density of a convergent integration substrate — the largest systems reliably dominating regardless of integration architecture — the attractor claim collapses into a scale claim and the convergence-fan shape carries no design content beyond "make it bigger."

**Catalog impact.** The cluster adds three `draft` NEXIs and no canonical promotion: Roth 2015 is currently the sole strong exponent of each member pattern, so all three remain single-source and draft pending a second corroborating source. The cluster's `convergent-integration-hub` is nonetheless canonical-_eligible_ — it is co-supported by the conserved-integrative-substrate evidence in [Nordli & Todd 2022] — but promotion is deferred as a deliberate curation decision rather than taken as an automatic consequence of ingestion. The cluster also sharpens a catalog-wide disambiguation: its hub pattern (the convergent _recurrence_ of an integration locus across independent systems) is explicitly distinguished from the distributed-social-cognition cluster's `multi-modal-integration` pattern (the fusion of sensory channels _within_ one system).

**Limitations.** The cluster's evidence is comparative-neuroanatomical and correlational, not computational: Roth documents that intelligence co-occurs with a dense associative hub and with connectivity density, not a forward model or predictive-coding mechanism, so the engineering hypotheses are extrapolations from structural convergence rather than from a demonstrated computation. The source also carries an anthropocentric intelligence ranking in its conclusion that the cluster deliberately does not inherit — the catalog uses the convergence evidence, not the ranking. As with §6.6, cluster-level canonical status awaits multi-source evidence at the shape level.

**What the addendum demonstrates about the framework.** The fourth cluster tested whether the framework could absorb a _new_ shape; the fifth tests whether it can absorb an _opposed_ one. Convergence-fan is not merely another entry alongside isomorphism — it is its structural inverse, and the framework represents both without strain because shapes are defined by their composition mode and direction (§6.4), not by a linear scale of complexity. That the vocabulary accommodates opposition is a stronger result than that it accommodates variety: it suggests the shape taxonomy is tracking real structural distinctions rather than imposing an ordering the evidence does not support.
