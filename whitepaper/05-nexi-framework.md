...
The second canonical promotion is `action-selection-as-common-substrate` (version 1.0.0), which crossed the threshold on **cross-substrate triangulation**: vertebrate-CBGTC evidence from the action-selection literature [Nordli & Todd 2022], bacterial-molecular evidence from the acerebrate-decision-making literature [Nesin & Chandrankunnel 2025], and cnidarian-nerve-net evidence from the cognitive-capacity literature [Turner et al. 2026] converge on the same architectural function (default-inhibit / selective-disinhibit gating, goal-directed sequence construction, contextual reinforcement) implemented in three categorically different substrates. The promotion sets a precedent the catalog had not previously needed: cross-substrate evidence — the same architectural pattern observed in molecular regulatory networks, nerve nets, and centralised neural circuitry — can lift a pattern to canonical status by demonstrating substrate-independence-of-architectural-pattern. The third canonical promotion is `multi-modal-integration` (version 1.0.0, promoted 2026-05-17), which crossed the threshold on **three-architectural-scale convergence**: behavioural-network evidence from the wild zebra finch communication-network literature [Hagedoorn et al. 2026], molecular-substrate evidence from the bacterial decision-making literature [Nesin & Chandrankunnel 2025] (the ~10,000-chemoreceptor per-cell sensor array that integrates light, pH, temperature, sugars, and oxygen into a single regulatory state), and circuit-level cross-phyletic evidence from the comparative-neuroanatomy literature [Roth 2015] (multi-modal integration centres independently evolved in five non-homologous neural substrates with quantitative cytoarchitecture in honeybee mushroom body and octopus vertical lobe). The promotion strengthens the precedent set by `action-selection-as-common-substrate`: cross-architectural-scale convergence — the same architectural commitment observable at population-network, molecular-sensor, and neural-circuit scales — meets the canonical threshold in the strongest available form. The same ingestion [Roth 2015] simultaneously strengthened `action-selection-as-common-substrate` from v1.0.0 to v1.1.0 by adding a fourth substrate vertex (the convergent-divergent fan motif across five phyla) and produced one new draft NEXI, `expansion-readout-circuit`, which describes the substrate-architectural circuit motif (vault-internal terminology: *convergent-divergent fan*) that implements the multi-modal integration centre at the circuit level. The new draft NEXI is held outside any cluster pending a future *substrate-independent-cognition* cluster — the catalog's first deliberate orphan-by-design entry, awaiting a third independent source confirming the motif at the functional level before cluster seeding. The three canonical NEXIs together establish that the catalog's promotion threshold supports three distinct convergence shapes: two-source same-substrate (`niche-specification`), cross-substrate triangulation (`action-selection-as-common-substrate`), and cross-architectural-scale convergence (`multi-modal-integration`).
...
The artefact lives at <https://github.com/Ds-GerardoCastro/NEXI>. Section 6 walks through the three demonstration case studies — one per cluster shape (constellation, pipeline, composition) — that exercise the framework defined here against the methodology defined in Section 4. Section 6.6 then summarises the fourth cluster (Embodied Action Selection, isomorphism shape) which was ingested subsequently and exercises the framework's openness to new shapes.
