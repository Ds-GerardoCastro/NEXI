...
## 5.6 The three collection shapes
...
The second canonical promotion is `multi-modal-integration` (version 1.0.0), which crossed the threshold on **cross-phyletic co-sourcing**: the zebra finch communication-network evidence for acoustic-plus-spatial channel fusion [Hagedoorn et al. 2026] is joined by comparative-neuroanatomy evidence that a dedicated multimodal-integration centre has been independently selected across five non-homologous lineages — insect mushroom bodies, the octopus vertical lobe, the cichlid pallium, the corvid and psittacid avian pallium, and the mammalian isocortex [Roth 2015]. Behavioural evidence from one lineage and convergent structural evidence across five others meet at the same architectural claim: that mutually-confirming cross-channel integration is a recurrently-selected primitive rather than a lineage-specific accident. The two canonical NEXIs together establish that the catalog's promotion threshold is not merely "two papers" but rather "two structurally-distinct sources converging on the same architectural claim," with the strongest form of that convergence being cross-phyletic co-sourcing.
...
The artefact lives at <https://github.com/Ds-GerardoCastro/NEXI>. Section 6 walks through the three demonstration case studies — one per collection shape (constellation, pipeline, composition) — that exercise the framework defined here against the methodology defined in Section 4.
