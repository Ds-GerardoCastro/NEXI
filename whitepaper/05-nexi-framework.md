...
## 5.6 The five cluster shapes

The catalog's strongest structural finding is that clusters do not all have the same shape. Three distinct shapes emerged from the construction of the first three clusters, each carrying a different system-level claim, a different complementarity story, and a different relationship between its member patterns. A fourth shape — _isomorphism_ — emerged from the subsequent ingestion of a vertebrate-action-selection source [Nordli & Todd 2022], and a fifth — _convergence-fan_ — from the ingestion of a comparative-neuroanatomy source on the independent evolution of intelligence [Roth 2015]; both are documented at the end of this section.
...
**Isomorphism.** The Embodied Action Selection cluster (anchored to the cortico-basal ganglia-thalamo-cortical loop as a vertebrate-conserved action-selection mechanism, [Nordli & Todd 2022]) is an isomorphism: the same architectural mechanism is redeployed across distinct domains via _exaptation_. The cluster's four members — `action-selection-as-common-substrate` (the substrate-invariant function), `ecological-context-model` (the formalism layer with three numbered equations specifying context construction, behaviour selection, and goal achievement), `exaptation-architectural-reuse` (the generative principle by which one well-developed mechanism gets redeployed to new domains), and `heuristics-as-habits-fusion` (the concrete behavioural-instance demonstrating the cluster's predictive content) — all describe the same architectural function under different domain transformations rather than different functions composed together. The system-level hypothesis predicts that an architecture committing to the cluster (action selection as a first-class substrate-general primitive, formalised via the Ecological Context Model, with explicit exaptive reuse of the same selection mechanism across motor and cognitive domains, and heuristic decisions implemented as a subset of chunked-sequence habits) outperforms duplicate-module architectures on cross-domain benchmarks at lower marginal cost per added domain. The cluster's distinctive contribution is that "different patterns" turn out to be the same pattern under domain shift — a structurally distinct shape from constellation (mutual enabling), pipeline (sequenced stages), or composition (layered concurrent components).

**Convergence-fan.** The Convergent Intelligence Attractor cluster (anchored to a comparative synthesis of the independent evolution of complex brains and high intelligence across insects, cephalopods, teleosts, birds, and mammals, [Roth 2015]) is a convergence-fan: many non-homologous architectures, evolved separately from ancestors that had neither complex brains nor intelligent behaviour, independently arrive at the same functional attractor. It is the deliberate inverse of isomorphism — isomorphism is one conserved mechanism redeployed under domain shift; convergence-fan is many different mechanisms under one functional target. Its three members read off what every route shares (`convergent-integration-hub`, a dense multimodal associative locus), the topology itself (`convergence-fan-routes`, substrate-diverse routes to one function), and how to measure the shared invariant (`network-density-over-size`, connectivity and packing density rather than absolute scale). The system-level hypothesis predicts that systems targeting the substrate-invariant signature the convergence reveals reach comparable capability from architecturally diverse starting points at matched connectivity budget, while scaling absolute size without a convergent integration hub underperforms.

The five shapes are not exhaustive. They are what the present catalog has produced; future sources may produce shapes the framework has not yet encountered (lattice, hierarchy, swarm), and the framework is open to that. What the five shapes already establish is a stronger structural claim than any single cluster carries on its own: nature does not assemble adaptive behaviour in one canonical way, and the perceptron lineage's implicit assumption that all intelligence resolves to a homogeneous network of perceptrons is a templating bias the catalog can now put pressure on with concrete, peer-reviewable cases of five distinct shapes.
...
The artefact lives at <https://github.com/Ds-GerardoCastro/NEXI>. Section 6 walks through the three demonstration case studies — one per cluster shape (constellation, pipeline, composition) — that exercise the framework defined here against the methodology defined in Section 4. Section 6.6 then summarises the fourth cluster (Embodied Action Selection, isomorphism shape) and Section 6.7 the fifth (Convergent Intelligence Attractor, convergence-fan shape), both ingested subsequently, which together exercise the framework's openness to new shapes.
