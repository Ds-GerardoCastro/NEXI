...
The illustrative table is not exhaustive, and the engagement that this paper proposes is no longer purely prospective. The NEXI catalog presented in Section 5 contains, at the time of writing, thirteen architectural patterns drawn from four peer-reviewed sources: research on the social-communication networks of wild zebra finches [Hagedoorn et al. 2026], on the cognitive-capacity limits of non-human animals [Turner et al. 2026], on collective decision-making in bacterial communities without nervous systems [Nesin & Chandrankunnel 2025], and on the convergent evolution of dedicated multimodal-integration centres across non-homologous lineages [Roth 2015]. These four sources sit at four different positions relative to the table above — network-level cognition, capacity-limited sensory and cognitive integration, acerebrate computation, and cross-phyletic comparative neuroanatomy — and each has produced a coherent _collection_ of mutually reinforcing patterns rather than a single isolated primitive. Roth 2015 is the catalog's strongest case that _the same architecture has been independently selected across non-homologous substrates_: a dedicated integration centre recurs convergently in insect mushroom bodies, the octopus vertical lobe, the cichlid pallium, the corvid and psittacid avian pallium, and the mammalian isocortex, none of which inherit the design from a common ancestor. The diversity of collection _shapes_ that has emerged (Section 5) is itself a finding: nature does not assemble adaptive behaviour in one canonical way, and the perceptron lineage has not yet engaged the alternatives at the level of design.

Two of these sources sharpen a question the table above leaves implicit. The bacterial-molecular case [Nesin & Chandrankunnel 2025] implements decision-making architecture without any nervous system; the cross-phyletic case [Roth 2015] shows a dedicated integration architecture independently selected across five non-homologous neural lineages. Each is _universal_ within its substrate scale. The position the program takes is **substrate-independence-of-architectural-pattern, not substrate-irrelevance**: the architectural function is the substrate-general invariant; the substrate is one parameter among many, and different substrates produce architecturally-equivalent patterns. AI systems matching the architectural pattern are matching nature's deep invariant, regardless of whether the implementation substrate resembles bacterial regulatory networks or one of several non-homologous neural architectures. This stance is operationalised in the catalog's _substrate-independent-cognition_ collection.
...
**At the collection level.** Each collection in NEXI commits to a _system-level falsifiable hypothesis_: a testable claim about the synergistic effect of its member patterns when adopted together. If subsetting a collection produces no measurable degradation beyond the additive sum of individual-pattern losses, the collection's synergy hypothesis is refuted — the patterns remain individually useful, and the catalog does not depend on the synergy holding. The collection-level claim is the paper's central falsifiable prediction, offered as a hypothesis to test rather than an established result. Three of the catalog's four collections have each yielded a distinct system-level claim and a distinct structural _shape_ (the fourth, _substrate-independent-cognition_, is newly seeded and single-source and is not yet assigned a distinct shape): **Distributed Social Cognition** collection (constellation shape, anchored to wild zebra finch communication) predicts that subsetting its co-dependent patterns degrades referential robustness beyond linear loss; the **Bounded Cognitive Architecture** collection (pipeline shape, anchored to capacity-limit research) predicts that downstream pattern stages fail without upstream capacity limits being honoured rather than circumvented; and the **Acerebrate Decision-Making** collection (composition shape, anchored to bacterial decision-making) predicts that adaptive collective behaviour is recoverable from a small set of substrate-independent components rather than from any specific molecular implementation. Each system-level claim is independently refutable.
